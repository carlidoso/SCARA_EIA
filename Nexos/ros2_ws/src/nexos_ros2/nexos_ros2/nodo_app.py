...
def main(args=None):
    ...
    nexos_app = NexosApp()
    main_app = MainApp(req_motor_ang1_publisher= nexos_app.req_motor_ang1_publisher, req_motor_ang2_publisher= nexos_app.req_motor_ang2_publisher, efector_final= nexos_app.efector_final, tipo_pose= Pose, tipo_vector= Vector3, nodo= nexos_app)
    try:
        ...
        nexos_app.get_logger().warning("Se interrumpio desde consola")
    finally:
        if rclpy.ok():
